  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      <button
        ...
        className={`w-full flex items-center justify-between px-3 py-2.5 bg-white border ${
      ...
      >
        <span className="truncate">
          {selectedOption ? selectedOption.label : <span className="text-slate-500 font-medium">{placeholder}</span>}
        </span>
      ...
      {isOpen && (
        <div className="absolute z-50 mt-1 w-full bg-white border border-slate-200 rounded-lg shadow-xl overflow-hidden max-h-60 flex flex-col">
          <div className="p-2.5 border-b border-slate-100 bg-slate-50 flex items-center gap-2">
            ...
            <input
              type="text"
              className="w-full bg-transparent text-sm text-slate-900 focus:outline-none placeholder-slate-500"
              placeholder="Search..."
              value={searchTerm}
          ...
          <div className="overflow-y-auto max-h-48 p-1">
            {filteredOptions.length === 0 ? (
              <div className="py-3 px-4 text-sm text-center text-slate-500">No matching items found</div>
            ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col text-left truncate">
                      ...
                      {opt.sublabel && <span className="text-xs text-slate-600 font-medium">{opt.sublabel}</span>}
                    </div>
